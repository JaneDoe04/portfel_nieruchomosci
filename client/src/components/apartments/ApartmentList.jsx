...
export default function ApartmentList() {
  ...
  const formatPrice = (p) => (p != null ? `${Number(p).toLocaleString('pl-PL')} zł` : '–');
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="text-left py-4 px-6 text-sm font-semibold text-slate-700">Tytuł / Adres</th>
                  <th className="text-left py-4 px-6 text-sm font-semibold text-slate-700">Status</th>
                  <th className="text-right py-4 px-6 text-sm font-semibold text-slate-700">Cena</th>
                  <th className="text-right py-4 px-6 text-sm font-semibold text-slate-700">m²</th>
                  <th className="text-left py-4 px-6 text-sm font-semibold text-slate-700">Koniec umowy</th>
                ...
                {apartments.map((apt) => (
                  ...
                  >
                    <td className="py-4 px-6">
                      ...
                      <div className="text-sm text-slate-500">{apt.address}</div>
                    </td>
                    <td className="py-4 px-6">
                    ...
                    <td className="py-4 px-6 text-right text-slate-600">{apt.area ?? '–'}</td>
                    <td className="py-4 px-6 text-slate-600">{formatDate(apt.contractEndDate)}</td>
                    <td className="py-4 px-6">
